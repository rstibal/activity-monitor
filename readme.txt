...
= 2.9.41 =
* Fixed: in the light version of every style, the background colour now fills the whole page edge to edge, as the dark version already did.
...
* Added: five more styles: Forest, Plum, Graphite, Solarized and Nord, each in light and dark.

= 2.9.38 =
* Added: a Style picker next to the Light/Dark toggle with five styles: Ledger Console (the existing look), WordPress (core admin colors and system font), Harbor, Paper, and High Contrast. Each works in light and dark, and your choice is remembered per user.
...
= 2.9.35 =
* Added: post and page edits now also record changes to author, publish date, parent, excerpt, comment/ping status, menu order, and whether a password was added or removed. Previously only title, content, status and slug were tracked.
...
= 2.9.32 =
* Added: plugin and theme installs or updates that fail (bad package, copy error, or a failed background auto-update) are now logged as warnings. Failed downloads are not covered.

= 2.9.31 =
* Added: switching auto-updates on or off for an individual plugin or theme is now logged. Turning them off is logged as a warning.
...
= 2.9.28 =
* Added: giving a user an additional role, or removing one of their roles, is now logged (User Role Added / User Role Removed). Previously only a wholesale role swap was recorded, so quietly adding Administrator to an Editor left no trace.
...
= 2.9.25 =
* Added: changes to four more site options are now logged — search-engine visibility (Discourage search engines), permalink structure, timezone, and site language (AM_Logger_Options).

= 2.9.24 =
* Added: on multisite, removing a user from a site is now logged. Adding a user to a site was already logged, but removal left no trace.

= 2.9.23 =
* Added: theme deletions and theme installs are now logged (AM_Logger_Themes). Plugin deletion and installs were already logged; themes only recorded switches and updates.
...
= 2.8.19 =
* Changed: removed the leftover margin around the search box in the Activity Log's top toolbar (carried over from when it sat as its own element above the tablenav) now that it sits inside the tablenav's top row in place of pagination.
...
* Changed: the "Update Now" cooldown is 1 minute instead of 5. 5 minutes meant every failed attempt cost a genuine 5-minute wait before the next retry could even be tried.
